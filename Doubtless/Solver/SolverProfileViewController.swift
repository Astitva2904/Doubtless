...
final class SolverProfileViewController: UIViewController {
    ...
    private func getGreeting() -> String {
        ...
        case 8..<12:
            return ["Good morning,", "Hello sunshine,", "Ready to solve?"].randomElement()!
        case 12..<14:
            return ["Good afternoon,", "Afternoon grind,", "Post-lunch hero,"].randomElement()!
        case 14..<17:
            return ["Keep it up,", "Crushing it,", "Afternoon push,"].randomElement()!
        case 17..<20:
            return ["Good evening,", "Evening solver,", "Winding down?"].randomElement()!
        case 20..<23:
            return ["Hey night owl,", "Burning the midnight oil,", "Late night solver,"].randomElement()!
        default:
            return ["Still up?", "Hello night owl,", "Midnight genius,"].randomElement()!
        }
    }
        ...
        Task {
            do {
                if let currentUser = try await SupabaseManager.shared.getCurrentUser() {
                    let name = currentUser.userMetadata["name"]?.stringValue ?? "Solver"
                    
                    DispatchQueue.main.async {
                        self.nameLabel.text = name
                        // Load year/class info dynamically
                    ...
                    do {
                        let daysTotal = 365
                        let stats = try await SupabaseManager.shared.fetchStreakStats(solverName: name, daysBack: daysTotal)
            ...
            // Fetch Solver Earnings
            do {
                let (total, _) = try await CreditsManager.shared.fetchSolverEarnings()
                DispatchQueue.main.async {
                    self.earningsLabel.text = String(format: "₹%.2f", total)
                }
            } catch {
                print("Failed to fetch solver earnings:", error)
                DispatchQueue.main.async {
    ...
    private func setupProfileSection() {
        ...
        nameLabel.text = "Solver" // Default until fetched
    ...
    private func setupCards() {
        ...
        // Give the earnings label an initial value
        earningsLabel.text = "₹..."
        earningsLabel.textColor = .systemGreen

        let earningsRow = makeRowButton(
            title: "Total Earnings",
            customValueLabel: earningsLabel,
            icon: "banknote"
        )

        let withdrawButton = makeRowButton(
            title: "Withdraw Earnings",
            icon: "arrow.up.right.circle.fill",
            action: #selector(openWithdrawal)
        )
    ...
    // MARK: - Actions
    @objc private func openWithdrawal() {
        navigationController?.pushViewController(SolverWithdrawalViewController(), animated: true)
    }

    @objc private func openNotifications() {
        navigationController?.pushViewController(SolverNotificationsViewController(), animated: true)
    }


    
    @objc private func openSettings() {
        navigationController?.pushViewController(SolverSettingsViewController(), animated: true)
    }
    @objc private func openFAQs() {
        navigationController?.pushViewController(FAQViewController(type: .solver), animated: true)
    }
    ...
    private func makeRowButton(
        ...
        if let custom = customValueLabel {
            validLabel = custom
            validLabel?.font = .systemFont(ofSize: 16, weight: .semibold)
            validLabel?.textColor = .label
        } else if let val = value {
            let newlyCreated = UILabel()
            newlyCreated.text = val
            newlyCreated.font = .systemFont(ofSize: 16, weight: .semibold)
            newlyCreated.textColor = .label
            validLabel = newlyCreated
        } else {
            validLabel = nil
        }

        let subviews: [UIView]
        if let vLabel = validLabel {
            subviews = [iconView, titleLabel, spacer, vLabel]
        } else {
            subviews = [iconView, titleLabel, spacer]
        }
