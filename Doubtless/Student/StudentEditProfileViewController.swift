...
final class StudentEditProfileViewController: UIViewController {
            ...
            do {
                if let user = try await SupabaseManager.shared.getCurrentUser() {
                    ...
                    DispatchQueue.main.async {
                        ...
                        if let c = meta["class"]?.stringValue, !c.isEmpty {
                            self.classButton.setTitle(c, for: .normal)
                            self.classButton.setTitleColor(.label, for: .normal)
                        }
                    }
    ...
    private func makeProfileHeader() -> UIView {
        ...
        cameraButton.backgroundColor = .systemOrange
    ...
    private func setupFields() {
        ...
        configureDropdownButton(classButton, placeholder: "Select class")

        stackView.addArrangedSubview(makeLabeledButton(title: "Gender", button: genderButton))
        stackView.addArrangedSubview(makeLabeledButton(title: "Currently in", button: classButton))

        genderButton.addTarget(self, action: #selector(selectGender), for: .touchUpInside)
        classButton.addTarget(self, action: #selector(selectClass), for: .touchUpInside)
    }
    ...
    @objc private func saveTapped() {
        ...
        Task {
            do {
                ...
                let studentClass = classButton.title(for: .normal)
                if studentClass != "Select class" { newMetadata["class"] = try AnyJSON(studentClass ?? "") }
    ...
    @objc private func selectClass() {
        showActionSheet(
            title: "Currently in",
            options: ["Class 9", "Class 10", "11th", "12th", "Dropper"]
        ) { [weak self] value in
...
extension StudentEditProfileViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
